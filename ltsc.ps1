...
Function LogScript {
	$ltscFolder = "C:\Temp\ltsc"
	If (Test-Path $ltscFolder) {
		Write-Output "$ltscFolder exists. Skipping..."
	}
	Else {
		Write-Output "The folder '$ltscFolder' doesn't exist. This folder will be used for storing logs created after the script runs. Creating now."
		Start-Sleep 1
		New-Item -Path "$ltscFolder" -ItemType Directory -Force -ErrorAction SilentlyContinue
		Write-Output "The folder $ltscFolder was successfully created."
	}
	Start-Transcript -OutputDirectory $ltscFolder
}
...
Start-Sleep 1
$startTime = Get-Date
Checkpoint-Computer -Description "<System Restore Point dated $startTime before running LTSC script.>" -RestorePointType "MODIFY_SETTINGS"
Start-Sleep 1
